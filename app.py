...
def detect_variable_types(df):
    ...
    for col in df.columns:
        ...
        # Vérifier si c'est numérique d'abord (priorité aux nombres)
        if pd.api.types.is_numeric_dtype(series):
            n_unique = series.nunique()
            n_total = len(series.dropna())
            # Si peu de valeurs uniques et entiers → peut être une catégorie
            if n_unique <= 10 and n_unique / n_total < 0.1 and series.dtype.kind in 'iu':
                types_dict[col] = "Qualitative"
            else:
                types_dict[col] = "Quantitative"
            continue

        # Vérifier si c'est une date (texte qui ressemble à une date)
        try:
            # Ne pas convertir les strings courtes (noms, catégories)
            sample = series.dropna().astype(str).head(20)
            if sample.str.len().mean() > 6:  # Dates sont généralement plus longues
...
def create_bar_chart(df, x_col, y_col=None, title=""):
    if y_col and y_col in df.columns:
        fig = px.bar(df, x=x_col, y=y_col, title=title, color=x_col, template="plotly_white")
    else:
        counts = df[x_col].value_counts().reset_index()
        counts.columns = [x_col, 'Count']
        fig = px.bar(counts, x=x_col, y='Count', title=title, color=x_col, template="plotly_white")
    fig.update_layout(showlegend=False)
    return fig


def create_histogram(df, col, title=""):
    fig = px.histogram(df, x=col, title=title, nbins=20, template="plotly_white",
...
def create_scatter(df, x_col, y_col, title=""):
    fig = px.scatter(df, x=x_col, y=y_col, title=title, template="plotly_white", opacity=0.7,
...
def create_boxplot(df, x_col, y_col, title=""):
    fig = px.box(df, x=x_col, y=y_col, title=title, template="plotly_white", color=x_col)
    fig.update_layout(showlegend=False)
    return fig
...
def create_line_chart(df, x_col, y_col=None, title=""):
    ...
    if y_col and y_col in df_temp.columns:
        fig = px.line(df_temp.sort_values(x_col), x=x_col, y=y_col, title=title,
                      template="plotly_white", markers=True)
    else:
        counts = df_temp.groupby(x_col).size().reset_index(name='Count')
        fig = px.line(counts.sort_values(x_col), x=x_col, y='Count', title=title,
                      template="plotly_white", markers=True)
    return fig
...
def create_pie_chart(df, col, title=""):
    ...
    fig = px.pie(counts, names=col, values='Count', title=title, template="plotly_white", hole=0.4)
    return fig
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("Charger un fichier (CSV, Excel)", type=['csv', 'xlsx', 'xls'])
    st.markdown("---")
...
if df is not None:
        ...
        with filter_col1:
            if qual_cols:
                ...
                if filter_qual != "Aucun":
                    ...
                    if filter_val:
                        df_filtered = df_filtered[df_filtered[filter_qual].isin(filter_val)]

        with filter_col2:
            if quant_cols:
                filter_quant = st.selectbox("Filtrer par variable quantitative", ["Aucun"] + quant_cols)
                if filter_quant != "Aucun":
                    min_val = float(df_filtered[filter_quant].min())
                    max_val = float(df_filtered[filter_quant].max())
    ...
    with tab3:
        ...
        with filt_col3:
            if date_cols:
                ...
                if date_filter != "Aucun":
                    df_viz[date_filter] = pd.to_datetime(df_viz[date_filter], errors='coerce')
                    valid_dates = df_viz[date_filter].dropna()
        ...
        st.markdown(f"<h3 style='text-align:center; color:#e74c3c;'>📊 {dyn_title}</h3>", unsafe_allow_html=True)
        ...
        if chart_type == "🤖 Auto (recommande)":
            ...
            if y_var == "Aucune":
                ...
                if x_type == "Quantitative" and y_type == "Quantitative":
                    ...
                    fig = px.imshow(crosstab, text_auto=True, aspect="auto",
                                    title=f"Tableau croise {x_var} x {y_var}", template="plotly_white")
                else:
                    fig = create_bar_chart(df_viz, x_var, y_var, title=f"{y_var} par {x_var}")

        elif chart_type == "📊 Bar Chart":
            fig = create_bar_chart(df_viz, x_var, y_var if y_var != "Aucune" else None, title=f"Bar Chart: {x_var}")
            ...
            fig = create_line_chart(df_viz, x_var, y_var if y_var != "Aucune" else None, title=f"Line Chart: {x_var}")
